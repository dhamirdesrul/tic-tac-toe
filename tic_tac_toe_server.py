...
class TTTServerGame(TTTServer):
	...
	def __main_loop(self):
		...
		while True:
			...
			logging.info("[" + str(datetime.datetime.now()) + "] - Received connection from " + str(client_address) + "\n")
			...
			try:
				...
				logging.info("[" + str(datetime.datetime.now()) + "] - Failed to create thread.\n")
	...
	def __client_thread(self, player):
		...
		try:
			...
			if(player.recv(2, "c") != "1"):
				# An error happened
				logging.info("[" + str(datetime.datetime.now()) + "] - Client " + str(player.id) + " didn't confirm the initial message.\n")
				print("Client " + str(player.id) + 
			...
			while player.is_waiting:
				...
				if(match_result is None):
					...
					try:
						...
						logging.info("[" + str(datetime.datetime.now()) + "] - Game between " + str(new_game.player1.id) + " and " + str(new_game.player2.id) + " is finished unexpectedly.\n")
						print("Game between " + str(new_game.player1.id) + 
							" and " + str(new_game.player2.id) + 
							" is finished unexpectedly.");
					# End this thread
					return;
		except:
			logging.info("[" + str(datetime.datetime.now()) + "] - Player " + str(player.id) + " disconnected.\n")
	...
	def matching_player(self, player):
		...
		try:
			for p in self.waiting_players:
				if (p.password == player.password and p.is_waiting and p is not player):
...
class Player:
	...
	def __connection_lost(self):
		...
		logging.info("[" + str(datetime.datetime.now()) + "] - Player " + str(self.id) + " connection lost.\n")
...
class Game:
	...
	def start(self):
		...
		# self.player1.receive_password();
		self.player2.send_match_info();
		# self.player2.receive_password();

		# Print the match info onto screen 
		logging.info("[" + str(datetime.datetime.now()) + "] - Player " + str(self.player1.id) + " is matched with player " + str(self.player2.id) + "\n")
		print("Player " + str(self.player1.id) + 
	...
	def move(self, moving_player, waiting_player):
		...
		if(self.board_content[move - 1] == " "):
			...
			logging.info("[" + str(datetime.datetime.now()) + "] - Player " + str(moving_player.id) + " is attempting to take a position that's already " + "been taken.\n")
		...
		if(result >= 0):
			...
			if(result == 0):
				...
				logging.info("[" + str(datetime.datetime.now()) + "] - Game between player " + str(self.player1.id) + " and player " + str(self.player2.id) + " ends with a draw.\n")
			...
			if(result == 1):
				...
				logging.info("[" + str(datetime.datetime.now()) + "] - Player " + str(self.player1.id) + " beats player " + str(self.player2.id) + " and finishes the game.\n")
...
def main():
	...
	try:
		...
		logging.info("[" + str(datetime.datetime.now()) + "] - Server critical failure.\n" + str(e) + "\n")
		print("Server critical failure.\n" + str(e));
